...
exports.run = (client, message, args) => {
  ...
  if (validUnlocks.includes(time)) {
    message.channel.overwritePermissions(message.guild.id, {
      ...
      message.channel.send('Kanal kilidi açıldı. İstediğiniz Zaman Kullanabilirsiniz.');
    ...
    message.channel.overwritePermissions(message.guild.id, {
      ...
      message.channel.send(`Kanal kilitlendi. ${ms(ms(time), { long:true })}`).then(() => {
        ...
        client.lockit[message.channel.id] = setTimeout(() => {
          message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: null
          }).then(message.channel.send('Kanalın kilidi açıldı. İstediğiniz Zaman Kullanabilirsiniz.')).catch(console.error);
          delete client.lockit[message.channel.id];
        }, ms(time));
